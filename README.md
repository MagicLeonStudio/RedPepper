...
### v0.0.7-patch (2026-07-17)

本次补丁完成 Phase 2「账户维度拆分」第一阶段收口，覆盖后端兼容迁移、接口行为修复与前端筛选落地。

**账户维度模型与迁移**
- 持仓模型与 schema 统一引入 `account_type`、`account_name`、`account_provider` 三个维度字段
- SQLite 增量迁移补齐账户维度列，并对历史数据执行回填（基金/理财/钱包等账户特征优先识别）
- 导入链路同步账户维度字段，保持新旧数据兼容

**持仓接口行为修复**
- 修复创建/更新时账户维度被默认值覆盖的问题：仅在用户显式传入时保留传值，否则根据 `account` / `type` 自动推断
- 修复更新场景中 `account` 或 `type` 变化后账户维度残留旧值（如“证券/中信”）的问题
- 持仓列表与汇总链路按账户维度统一：支持 `account_type` / `account_provider` 过滤并保持统计一致

**前端持仓页筛选与展示**
- 新增账户关键字、账户类型、券商筛选控件（支持应用/重置，关键字回车即可触发筛选）
- 持仓表账户列升级为“账户(类型/券商)”展示，降低多账户场景识别成本
- 前端服务层补齐账户维度参数透传，与后端查询口径保持一致
...
本版本聚焦「持仓 / 观察池导入链路的彻底收口」，将 v0.0.5 期间进行中的本地 OCR、双模型重试、强制补码等能力正式定版，并修复了一批导入准确性与数据一致性问题。

**持仓截图导入**
- 修复本地 OCR 分支因未定义 `_extract_security_code` 导致的 500 崩溃（导入长期不可用的根因）
- 解决券商表格首列「明细」勾选框导致的整列左移错位：改为 CSV 列解析与券商/文本解析并跑、按有效 6 位代码数择优
- 新增同花顺基金卡片解析 `_parse_ths_fund_cards_text`：按「名称（代码）/ 指标表头 / 数值」三行还原份额、市值、净值、盈亏，并反推成本净值
- 名称清洗（去除 OCR 残留尾部括号）与噪声过滤（账户/产品标签、纯数值、`null`、版块标签）

**账户识别稳健性**
- 本地 OCR 改用整图识别，保留顶部「同花顺钱包/账户资产」表头用于账户判定
- `_is_ths_fund_source` 关键词补充「同花顺基金」，并加固 DeepSeek 归一化 prompt，避免基金账户被改回中信证券、进而误删跨账户持仓

**观察池导入**
- 名称解析跳过「代码/板块标志单元格」，修复「融」「陆」被当作名称
- 剔除同花顺概念/板块伪代码（`88xxxx`），并禁止 LLM 编造候选库外代码（仅接受真实存在的代码 + 可交易性校验）
- 基金长名跨行合并（接C / 起式联接C / QDII-A），过滤导航/表头/数值噪声行

**持仓 → 观察池同步**
- 持仓导入同步生成的观察池条目，自动补全行业、观察理由、触发条件（`enrich_watchlist_db_items`，只补空缺不覆盖）

**前端 / 文档**
- 观察池「已买入」状态改为醒目黄色
- HELP.md 补充持仓与观察池导入的完整运行逻辑说明与 Mermaid 流程图

### v0.0.5-patch (2026-06-29)

本次补丁重点是“截图 OCR 在网络超时与模型兼容异常场景下的可用性收口”。

**本地 OCR 兜底（RapidOCR）**
- 新增本地 OCR 模块，使用 `rapidocr-onnxruntime` 进行截图文本识别，并支持切片 + 预处理（灰度、对比度增强、二值化）
- 新增 `ai.local_ocr` 配置项（`enabled`、`max_slices`、`prefer_local`），默认启用且本地优先
- 持仓截图提取链路升级：本地优先 -> 远端双模型 -> 最终降级 CSV（至少保留标准表头），避免 422 硬失败
- 观察池截图提取链路升级：本地优先 -> 远端双模型；全链路失败时降级为空响应，避免 504 直接中断导入流程

**导入鲁棒性增强（v0.0.6 进行中）**
- 截图 OCR 远端链路补齐双模型重试与降级：DeepSeek/Kimi 的整图 + 切片重试
- 观察池无代码行新增“强制补码回填”：先本地候选匹配（持仓 + 观察池），再 LLM 补码

**本地 OCR 表格化输出（v0.0.6 进行中）**
- 修复行重建的“滑动平均锚点”缺陷：密集表格会因锚点漂移把整页 token 合并成单行，改为稳定锚点 + 自适应行高阈值（按 token 框中位高度）逐行聚类
- 识别结果由单行 `.txt` 改为按视觉行还原、列间以制表符分隔的表格文本（TSV），更适配 CSV 入库与模型后处理；下游解析器以 `\t` 切列，天然兼容
- 打分逻辑改为奖励多列表格行数，提升多变体择优的稳定性

**回归验证**
- `stock-portfolio.png`：由失败（422）变更为成功，返回 `provider=local_ocr`
- `stock-watchlist-1.png`：由失败（504）变更为成功，返回 `provider=local_ocr` 且可提取条目
- 表格化 5 图回归：中信观察池由塌陷单行恢复为 50/4 行（含 48/2 个代码），持仓与基金表分别为 13/18/43 行，逐行列对齐正常

### v0.0.5-patch (2026-06-16)

本次补丁重点是"投资简报自动化链路落地 + 持仓文本导入补全增强"（不升级版本号）。

**投资简报自动化链路**
- 新增简报自动化模型与运行记录：支持开盘 / 午间 / 收盘会话类型、状态跟踪、失败原因留痕
- 新增简报上下文构建与生成服务：聚合持仓、观察池、交易日志、事件日历、知识库，为 AI 生成提供统一输入
- 新增后端 `/context-preview`、`/generate`、`/runs`、`/run-due` 接口，支持上下文预览、手动生成、运行历史查看与定时触发
- 前端简报页面新增一键生成、运行状态面板、“查看上下文 / 查看运行详情”能力；主窗口新增后台轮询自动触发

**持仓文本导入增强（中信样例修复）**
- 修复原始券商文本在无代码场景下无法入库的问题，文本解析改为允许名称优先入库
- 剪贴板文本导入补充 CSV -> 纯文本回退，避免“解析成功但 0 行入库”直接失败
- 新增持仓文本导入预处理：基于持仓库 + 观察池候选做本地模糊匹配，补全证券代码与标准名称；未命中时由 DeepSeek 兜底补全
- 常见简称如“机器人E”“科创50”“纳指ETF”“科创芯片”可自动归一化到标准名称并同步落库
...
| OCR 能力 | 多模态 LLM + RapidOCR | 云端视觉识别 + 本地兜底识别 |
